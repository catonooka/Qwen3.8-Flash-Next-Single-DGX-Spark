...
class SpecDecodeBaseProposer:
    ...
    def propose(
        ...
        # F4b (fork 2026-09-12): thread the acceptance-aware sample row index
        # to the MTP drafter via the module-global stash in logits_processor.
        # One dict write, eager code, no-op when the stash is absent.
        try:
            from vllm.model_executor.layers.logits_processor import _F4B_STASH

            if _F4B_STASH is not None and token_indices_to_sample is not None:
                _F4B_STASH["tits"] = token_indices_to_sample
        except Exception:
            pass
