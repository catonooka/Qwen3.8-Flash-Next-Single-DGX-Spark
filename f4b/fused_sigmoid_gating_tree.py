...
@triton.heuristics(
    {
        ...
        # F4b-tree (fork 2026-09-13): explicit per-group start-state slots
        "IS_TREE": lambda args: args["initial_state_indices"] is not None,
    }
)
...
def fused_sigmoid_gating_delta_rule_update_kernel(
    ...
    if USE_INITIAL_STATE:
        if IS_TREE:
            # F4b-tree: each cu_seqlens group starts from an explicitly-provided
            # parent state slot (sibling branches share the SAME parent slot —
            # read-only here, per-position saves via ssm_state_indices).
