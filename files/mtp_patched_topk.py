...
def _attach_draft_vocab(model: nn.Module) -> None:
    ...
    try:
        model._draft_topk_k = int(os.environ.get("VLLM_MTP_DRAFT_TOPK", "0").strip() or 0)
    except ValueError:
...
class Qwen3_8FlashNextMTP(nn.Module, SupportsPP, Qwen3_8FlashNextMixtureOfExperts):
    ...
    def _topk_draft_argmax(self, hidden_states: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        """F4a-lite: cycle-local top-K draft sampling (zero output risk).

        Draft quality only affects acceptance, never emitted tokens (greedy
        verify decides those). Within one MTP cycle of `1 + num_spec` calls to
        get_top_tokens, call 0 pays the full slice GEMV and caches the top-K
        slice-row indices of its own draft distribution; calls 1..N gather only
        those K rows (K x hidden bytes) instead of the whole slice. Between
        cycles the cache is refreshed by that cycle's call 0, so the index set
        always comes from the freshest full distribution available.

        Env gate: VLLM_MTP_DRAFT_TOPK (int, rows; unset/0 = stock path).
        """
        k = getattr(self, "_draft_topk_k", 0)
        if k <= 0 or weight.shape[0] <= k:
            logits = torch.nn.functional.linear(hidden_states.to(weight.dtype), weight)
            return logits.argmax(dim=-1)

        self._draft_topk_step = getattr(self, "_draft_topk_step", 0) + 1
    ...
    def get_top_tokens(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            k = 0
        if k > 0 and hidden_states.dim() == 2:
            argmax_rows = self._topk_draft_argmax(hidden_states, weight)
            return self._draft_id_to_target_id[argmax_rows].to(torch.long)
        logits = torch.nn.functional.linear(hidden_states.to(weight.dtype), weight)
        return self._draft_id_to_target_id[logits.argmax(dim=-1)].to(torch.long)


    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        # MXFP8-head surgery (2026-09-07): the main lm_head ships as fp8-e4m3 +
        # E8M0 scales in the derived checkpoint, but the DRAFTER stays a plain
        # BF16 head (its get_top_tokens path reads raw weight rows and the
        # draft-vocab slice expects a dense dtype). Dequantize in flight here:
        # when the fp8 weight has passed through, hold it until its per-32
        # scale rows arrive, then emit a dequantized BF16 tensor instead.
        _pending_fp8_head: dict[str, torch.Tensor] = {}
        ...
        def remap_weight_names():
            for name, weight in weights:
                ...
                if remapped_name is None:
                    continue
                if remapped_name == "lm_head.weight" and weight.dtype in (
                    torch.float8_e4m3fn,
                    torch.float8_e5m2,
                ):
                    _pending_fp8_head["lm_head.weight"] = weight
                    continue
                if (
                    remapped_name == "lm_head.weight_scale"
                    and "lm_head.weight" in _pending_fp8_head
                ):
                    q = _pending_fp8_head.pop("lm_head.weight").float()
                    # scale: uint8 E8M0 exponent (bias 127), one per 32-input block
                    s = torch.pow(2.0, weight.float() - 127.0)  # [V, K/32]
                    V, K = q.shape
                    deq = (
                        q.view(V, K // 32, 32) * s[:, :, None]
                    ).view(V, K).to(torch.bfloat16)
                    yield "lm_head.weight", deq
                    continue
