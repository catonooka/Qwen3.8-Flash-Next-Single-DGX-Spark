...
class LogitsProcessor(PluggableLayer):
    ...
    def __init__(
        ...
        # INT8 verify head (fork, W5 two-stage protocol): screen the full-vocab
        # logits with a per-row INT8 copy of the head, then rewrite the top-64
        # positions with exact BF16 row dots. Argmax is BF16-identical whenever
        # the BF16 top-1 sits inside the INT8 top-64 (measured 100.00% on the
        # real tensor; see research2/w5 + int8_head_bench.py). Env-gated.
        import os as _os
        try:
            self._int8_head = int(_os.environ.get("VLLM_INT8_VERIFY_HEAD", "0").strip() or 0)
        except ValueError:
            self._int8_head = 0
        self._int8_w = None
    ...
    def _int8_apply_head(
        self,
        lm_head: VocabParallelEmbedding,
        hidden_states: torch.Tensor,
    ) -> torch.Tensor:
        """Two-stage head: INT8 screen + exact BF16 top-64 rescore."""
        ...
        acc = torch._int_mm(hi8p, self._int8_w.t())  # [PAD, V] int32
        logits = acc.float()[:B] * ascale[:B] * self._int8_scale[None, :]
        # Stage 2: exact rescore of the top-64 rows with the BF16 head.
        top = logits.topk(64, dim=-1)  # values, indices
        w64 = self._int8_bf[top.indices.reshape(-1)].view(B, 64, H)
        hh = hidden_states.unsqueeze(1).expand(-1, 64, -1)
        exact = torch.einsum("bkh,bkh->bk", hh.float(), w64.float())
    ...
    def _apply_head(
        ...
        if (
            getattr(self, "_int8_head", 0)
            and lm_head.tp_size == 1
            and embedding_bias is None
            and hidden_states.dim() == 2
            and 0 < hidden_states.shape[0] <= 64
        ):
            if self._int8_w is None and not torch.cuda.is_current_stream_capturing():
                # Build once, eagerly (warmup run precedes graph capture).
                wref = lm_head.weight.data
                scale = (wref.abs().amax(dim=1).float() / 127.0).contiguous()
                w8 = (
                    (wref.float() / scale[:, None]).round().clamp(-127, 127)
                    .to(torch.int8)
                ).contiguous()
            ...
            if self._int8_w is not None:
                try:
                    ...
                    # Never fail a request over the fast head; fall through.
                    self._int8_head = 0
